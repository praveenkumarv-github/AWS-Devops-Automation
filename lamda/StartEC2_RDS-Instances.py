...
def get_instanceName(instanceid,client_region):
    ...
    for i in response["Tags"] :
        ...
        if str(tag_key) == "Name" :
            return tag_value
def startEc2instance(instanceid,client_region):
    response = client_region.start_instances(InstanceIds=[instanceid])
...
#EC2 block
def startEC2(ec2Instances,client_region) : 
    for instance in ec2Instances :
        try :
            ...
            if len(response["InstanceStatuses"] ) < 1 :
                startEc2instance(instance,client_region) #ec2.start
                state_res = instance_identification_Name+" is started"
...
#RDS block
def startRDS(rdsInstances,clientrds) : 
    for rds in rdsInstances : 
        try :
            rds_response = clientrds.describe_db_instances(DBInstanceIdentifier=rds) #get Status before start
            ...
            if str(pre_DBInstanceStatus) != "running" :
                start_rds_response = clientrds.start_db_instance(DBInstanceIdentifier=rds)  #RDS.instance.start

                rds_response = clientrds.describe_db_instances(DBInstanceIdentifier=rds) #get Status after start
                DBInstanceStatus = (rds_response["DBInstances"][0]["DBInstanceStatus"])
...
startEC2(ec2Instances_Singapore,clientec2_Singapore)
startRDS(rdsInstances_Singapore,clientrds_Singapore)
